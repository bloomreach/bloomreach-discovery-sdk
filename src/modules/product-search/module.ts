...
import {
  PaginationNode,
  ProductSearchData
} from '../../utils/object-mappers/product-search/interfaces/product-search-data'
...
export interface ProductSearchDependencies {
  template: string
...
export class ProductSearchModule {
  private data: ProductSearchDependencies
  ...
  constructor(options: ProductSearchDependencies) {
  ...
  static load(): ProductSearchModule | null {
    const config = ProductSearchConfig.build({q: ''})
  ...
  init(config: ProductSearchConfig): ProductSearchModule {
    ...
    if (urlParameters.has(config.get('defaultSearchParameter'))) {
      ...
      if (
        ...
        this.data.searchResultsContainerElement?.parentNode?.insertBefore(
          indicatorElement,
          this.data.searchResultsContainerElement.nextSibling
        )
      ...
      const callback = (mutationsList: MutationRecord[]) => {
          ...
          (mutationRecord) =>
            ...
            Array.from(mutationRecord.addedNodes).find(
              (node) =>
                ...
                (node as HTMLElement).classList.contains('blm-product-search')
            )
        )
        ...
        if (productListAdded) {
          ...
          const paginationContainer = document.querySelector(
            '.blm-product-search-pagination__pages'
          )
          if (paginationContainer) {
            paginationContainer.addEventListener('click', (event) => {
              ...
              if (clickedPaginationValue) {
                ...
                window.scrollTo(
                  0,
                  this.data.searchResultsContainerElement.offsetTop
                )
              }
            })
          }
          ...
          document
            .querySelectorAll(
              `.blm-product-search-filter-item:nth-child(-n+${initialNumberOfFacetValues})`
            )
          ...
          loadSwatch = function () {
            document
              .querySelectorAll(
                '.blm-product-search-swatch-image:nth-child(-n+1)'
              )
              .forEach((item: any) => (item.style.display = 'block'))

            document
              .querySelectorAll(
                '.blm-product-search-swatch-container__swatch:nth-child(-n+1)'
              )
              .forEach((item: any) => item.classList.add('active'))

            const results = document.querySelectorAll(
              '.blm-product-search__result'
            )
  ...
  async initiateScroll(
    ...
    if (urlParameters.has(config.get('defaultSearchParameter'))) {
      const parameters: ProductSearchConfigParameters = {
        q: urlParameters.get(config.get('defaultSearchParameter')) as string,
      ...
      const scrollResults = await ApiCore.getProductSearchData(
        (allParameters as unknown) as ProductSearchConfigParameters
      )
      ...
      const searchResults = document.querySelector(
        '.blm-product-search__results'
      ) as HTMLElement

      const infiniteScrollTemplate =
        config.get('productListTemplate') ||
    ...
    if (urlParameters.has(config.get('defaultSearchParameter'))) {
      ...
      */
      ...
      if (urlParameters.has(PARAMETER_NAME_SORT)) {
        parameters.sort = urlParameters.get(
          PARAMETER_NAME_SORT
        ) as SortByOptions
      }
      ...
      const results = await ApiCore.getProductSearchData(
        (allParameters as unknown) as ProductSearchConfigParameters
      )
      ...
      results.paginationData = ProductSearchModule.buildPaginationDataForResults(
        results
      )
