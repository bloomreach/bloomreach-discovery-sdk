...
test('it builds the correct config object for ProductSearch module', () => {
    utils_1.mocked(common_1.buildStaticProductSearchConfigParameters).mockReturnValue({
    ...
    const productSearchConfig = product_search_1.ProductSearchConfig.build({
        q: 'valid product search query'
    });
    expect(common_1.buildStaticProductSearchConfigParameters).toHaveBeenCalledTimes(1);
