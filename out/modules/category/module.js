...
const category_1 = require("../../config/category");
...
class CategoryModule {
    ...
    checkRequirements() {
        ...
        if (!this.data.resultsContainerElement) {
            throw new Error('You must provide search results container element for Category module');
        }
    }
    static load(categoryIdToLoad = '') {
        const config = category_1.CategoryConfig.build({ q: categoryIdToLoad });
        if (!categoryIdToLoad && !config.get('isCategoryPage')) {
            return null;
        }
        const resultsContainerElement = document.querySelector(config.get('selector'));
    ...
    init(config) {
        ...
        if (config.get('urlParameters').q ||
            ...
            if ((common_1.mobileView.matches || config.get('infiniteScroll')) &&
                ...
                (_b = (_a = this.data.resultsContainerElement) === null || _a === void 0 ? void 0 : _a.parentNode) === null || _b === void 0 ? void 0 : _b.insertBefore(indicatorElement, this.data.resultsContainerElement.nextSibling);
            }
            if (common_1.mobileView.matches || config.get('infiniteScroll')) {
            ...
            const callback = (mutationsList) => {
                const productListAdded = mutationsList.find((mutationRecord) => mutationRecord.type === 'childList' &&
                    Array.from(mutationRecord.addedNodes).find((node) => node.classList &&
                        node.classList.contains('blm-category')));
                if (productListAdded) {
                    ...
                    if (document.querySelector('.blm-category-sidebar')) {
                        const sidebarControlButtons = document.querySelectorAll('.blm-category-control-button--sidebar');
                        const sidebarControlButtonClickHandler = () => {
                            const sidebarContentElement = document.querySelector('.blm-category-sidebar-content');
                        ...
                        const facetCheckboxes = document.querySelectorAll('.blm-category-filter-item__checkbox');
                        if (facetCheckboxes) {
                            facetCheckboxes.forEach((checkbox) => {
                                checkbox.addEventListener('change', () => {
                                    ...
                                    const checkedCheckboxes = document.querySelectorAll('.blm-category-filter-item__checkbox:checked');
                    ...
                    const paginationContainer = document.querySelector('.blm-category-pagination__pages');
                    ...
                    // Facet
                    const loadMore = document.getElementsByClassName('blm-category-load-more');
                    ...
                    const loadMoreFacet = () => {
                        let i = 0;
                        for (const item of document.querySelectorAll(`.blm-category-filter:not([style*="display: block"])`)) {
                            if (i < numberOfDisplayedFacets) {
                                item.setAttribute('style', 'display: block');
                            }
                            i++;
                        }
                        if (document.querySelectorAll(`.blm-category-filter:not([style*="display: block"])`).length === 0) {
                    ...
                    document
                        .querySelectorAll(`.blm-category-filter-item:nth-child(-n+${initialNumberOfFacetValues})`)
                    ...
                    loadSwatch = function () {
                        document
                            ...
                            .forEach((item) => (item.style.display = 'block'));
                        document
                            .querySelectorAll('.blm-category-swatch-container__swatch:nth-child(-n+1)')
                            .forEach((item) => item.classList.add('active'));
                        const results = document.querySelectorAll('.blm-category__result');
                        results.forEach(function (result) {
                            const swatchContainer = result.querySelectorAll('.blm-category-swatch-container');
                            swatchContainer.forEach(function (swatchItems) {
                                const swatchItem = swatchItems.querySelectorAll('.blm-category-swatch-container__swatch');
                                swatchItem.forEach(function (swatch, indexSwatch) {
                                    swatch.addEventListener('mouseover', function () {
                                        ...
                                        const imageContainer = result.querySelectorAll('.blm-category-image-container');
                                        imageContainer.forEach(function (imageItems) {
                                            const imageItem = imageItems.querySelectorAll('.blm-category-swatch-image');
            ...
            observer.observe(this.data.resultsContainerElement, mutationObserverConfig);
        }
        return this;
    }
    ...
    initiateScroll(urlParameters, config) {
        return __awaiter(this, void 0, void 0, function* () {
            const parameters = {
                q: config.get('urlParameters').q ||
                    urlParameters.get(config.get('defaultSearchParameter')) ||
                    config.get('categoryId'),
            ...
            const scrollResults = yield core_1.ApiCore.getCategoryData(allParameters);
            ...
            const searchResults = document.querySelector('.blm-category__results');
            const infiniteScrollTemplate = config.get('productListTemplate') || CategoryModule.productListTemplate;
    ...
    initiateSearch(urlParameters, config) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            */
            ...
            const results = yield core_1.ApiCore.getCategoryData(allParameters);
            const scrollLoader = document.querySelector('.blm-scroll-indicator__loading');
            ...
            if (results.priceRanges) {
                ...
                const startValue = Number(results.priceRanges[0].start === '*' ? 0 : results.priceRanges[0].start);
            ...
            results.paginationData = CategoryModule.buildPaginationDataForResults(results);
            ...
            this.data.resultsContainerElement.innerHTML = ejs_1.default.render(this.data.template.replace('%%-PRODUCT_LIST_TEMPLATE-%%', config.get('productListTemplate') || CategoryModule.productListTemplate), results);
        });
    }
...
CategoryModule.template = `
  ...
  `;
CategoryModule.productListTemplate = `
